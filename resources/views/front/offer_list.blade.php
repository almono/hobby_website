...
<form action="{{url("category/$category")}}">
    ...
    @if (Request::has('sort') && (Request::get('sort') == 'nazwa-asc' || Request::get('sort') == 'nazwa-desc'))
        <input type="hidden" name="sort" value="{{Request::get('sort') }}"/>
    @endif

    @if( Request::has('custom_name') && Request::get('custom_name')!='')
        <input type="hidden" name="custom_name" value="{{ Request::get('custom_name') }}"/>
    @endif

    @if( Request::has('custom_town') && Request::get('custom_town')!='')
        <input type="hidden" name="custom_town" value="{{ Request::get('custom_town') }}"/>
    @endif

    @if( Request::has('sort_subcategory') && Request::get('sort_subcategory')!='')
        <input type="hidden" name="sort_subcategory" value="{{ Request::get('sort_subcategory') }}"/>
    @endif

</form>
</div>
    @foreach($items as $i)
        <div class="col-md-4 item-div item{{$i->id}}" style="padding-left: 0px; padding-right: 0px;">
            ...
            <div class="col-md-12 text-center item-name" id="with-hover" style="color: #303030; font-size: 12px !important; border-top: none;">@isset($i->city){{$i->city}}, @endisset {{$i->year}}r</div>
        </div>
    @endforeach

</div>
<div class="col-xs-12 text-center">
    <?php echo $items->appends(['sort' => Input::get('sort'), 'custom_name' =>Input::get('custom_name'), 'custom_town' => Input::get('custom_town'), 'sort_subcategory' =>Input::get('sort_subcategory')])->render(); ?>
</div>
